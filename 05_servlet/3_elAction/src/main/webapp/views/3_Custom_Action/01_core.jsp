...
<body>
	...
	<pre>
		* 변수 선언과 동시에 초기화 (c:set var="변수명" value="값" [scope="저장객체"]/)
		...
		- 해당 변수의 저장된 값을 어떤 scope에 setAttribute를 통해 담아둘 지 결정
	...
	<%-- addNum에 저장된 값을 출력 --%>
	<c:out value="${ addNum }"></c:out>
	<c:out value="${ addNum }" default="데이터 없음"></c:out>
	
	<%-- outData라는 변수에 "<b>강조태그!!!<b>" 값을 저장 --%>
	<c:set var="outData" value="<b>강조태그!!!</b>"></c:set>
	<c:out value="${ outData }"/> <%-- escapeXml 속성이 true(기본값) -> 출력데이터에 태그가 포함되어 있는 경우 해서되지 않음 --%>
	...
	<pre>
		...
		* 향상된 for문 - (c:forEach var="변수명" items="배열/컬랙션(순차적으로 접근하고자 하는 객체)"
	 		 ...
	 		 <c:choose>
		 		...
		 		<c:otherwise>
		 			<c:forEach var="p" items="${ pList }" varStatus="s">
		 				<tr>
		 					<%-- <td>${ s.index }</td> --%><%-- <td>${ s.index }</td>--%> <%-- index : 0부터 시작 --%>
	...
	<pre>
    	...
    	- 구분자를 통해서 분리된 각각의 값에 순차적으로 접근해 반복문 수행
