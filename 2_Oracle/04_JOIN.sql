...
-- 부서코드, 부서명 조회 (부서 정보가 저장된 데이블 : DEPARTMENT)
...
-- => NULL(EMPLOYEE_DEPT_CODE), 마케팅부(D3), 해외영업3부(D7), 국내영업부(D4) -> DEPARTMENT 데이터는 제외됨!
--         각 테이블에만 존재하는 값들로 조회에서 제외됨

-- 사원의 사번, 이름, 직급명을 조회 (직급 코드 컬럼 => EMPLOYEE: JOB_CODE, JOB: JOB_CODE)
-- 두 테이블의 컬럼명이 동일할 경우 별칭을 지어 줌으로써 구분할 수 있음!
...
-- * RIGHT JOIN : 두 테이블 중 오른쪽에 작성된 테이블 기준으로 확인
-- ** 오라클 구문 **
SELECT EMP_NAME, DEPT_TITLE, SALARY, SALARY*12
FROM EMPLOYEE, DEPARTMENT
WHERE DEPT_CODE(+) = DEPT_ID
...
WHERE E.MANAGER_ID = M.EMP_ID;       -- 사원의 사수번호, 사번을 조건으로 주어 사수 정보를 조회
...
-- 사번, 사원명, 부서명, 지역명 조회
SELECT * FROM EMPLOYEE;          -- 부서 코드 :  DEPT_CODE
SELECT * FROM DEPARTMENT;      --  부서 코드 : DEPT_ID, 지역코드: LOCATION_ID
SELECT * FROM LACATION;             -- 지역 코드 : LOCAL_CODE
