...
SELECT EMP_NAME 사원명, RPAD(SUBSTR(EMP_NO, 1,7 ), LENGTH(EMP_NO), '*') 주민번호
...
SELECT *
FROM EMPLOYEE
-- WHERE MONTHS_BETWEEN(SYSDATE, HIRE_DATE) >= 240;    -- 12 * 20
...
SELECT EMP_NAME 사원명, TO_CHAR(SALARY, 'L99,999,999') 급여  -- L: 언어 설정에 따라 화폐단위 표시
FROM EMPLOYEE;
...
--     주민번호 앞자리와 뒷자리의 합 조회
SELECT EMP_NAME 사원명, SUBSTR(EMP_NO, 1, 6) "주민번호 앞자리"
                    , SUBSTR(EMP_NO, 8) "주민번호 뒷자리" -- SUBSTR(EMP_NO, 8, 7)
                    , SUBSTR(EMP_NO, 1, 6) + SUBSTR(EMP_NO, 8) "주민번호 앞자리와 뒷자리의 합"
                    -- TO_NUMBER(SUBSTR(EMP_NO, 1, 6)( + TO_NUMBER(SUBSTR(EMP_NO, 8)) 생략해도 자동으로 형변환!
FROM EMPLOYEE
WHERE EMP_ID = 201;
...
/*
        COUNT( CASE
                                        WHEN STUBSTR(HIRE_DATE, 1, 2) = 99 THEN 1
                            END)    "1999년 입사 인원 수)"
*/
